<template>
  ...
  <div v-else class="outer">
    <div
      ...
      class="incubatorGroup control"
    >
      <div
        v-for="(value, key) in incubatorControl"
        :key="key"
        v-show="
          !(
            incubators[parseInt(incubatorControl.id)].mode === 0 &&
            key === 'dust'
          )
        "
        class="item"
...
export default {
  ...
  methods: {
    ...
    submit() {
      let incubatorControlGroup = {
        serialNumber: this.serialNumber,
        incubatorControls: this.incubatorControls,
      };
      this.incubatorControls.forEach((incubatorControl) => {
        if (
          incubatorControl.temperatureHigh < incubatorControl.temperatureLow ||
        ...
        if (
          incubatorControl.dust < 0 ||
          incubatorControl.dust > 1 ||
          incubatorControl.light < 0 ||
          incubatorControl.light > 1 ||
          incubatorControl.days < 0 ||
          incubatorControl.days > 1
        ) {
          throw new Error("");
        }
      });

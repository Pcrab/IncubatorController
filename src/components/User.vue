    ...
    <div class="outer">
      ...
      <div class="input_box" v-if="register">
        <input
          ...
          type="text"
          id="serialNumber"
          @blur="checkSerialNumber($event)"
          required
        />
        <label for="serialNumber">序列号</label>
      </div>

      <div v-if="errorContent !== ''" class="errorContent">
        {{ errorContent }}
      </div>
    ...
    btnClick() {
      ...
      if (this.register) {
        if (
          ...
          !isValidPassword(this.pwd1) ||
          this.pwd1 !== this.pwd2 ||
          !isValidSerialNumber(this.serialNumber)
        ) {
          return;
        }
        let user = {
          username: this.username,
          password: this.encrypt(this.pwd1),
